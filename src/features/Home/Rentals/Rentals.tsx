...
const Rentals = () => {
  ...
  const rentals: RentalProps[] = useSelector((state: RootState) => state.rentals.rentalsData);
  // Get the current array of filtered rentals (if any)
  const filteredRentals: RentalProps[] = useSelector((state: RootState) => state.rentals.filteredRentals);
      ...
      <div className="grid grid-cols-2 gap-0 md:w-[48rem] lg:bg-white lg:w-[61rem] xl:w-[69rem] m-auto">
        {rentalsToShow.map((rental) => (
          ...
          >
            <img
              src={rental.image}
              className="ml-2 xl:ml-0 min-h-[10rem] md:w-48 lg:w-56 xl:w-64 xl:h-40 xl:object-contain"
            />
            <div className="md:w-[9.6rem] md:h-40 lg:w-[13.7rem] xl:w-64 xl:h-40 ml-1 p-3">
              ...
              <p className="font-bold mt-1">
                {rental.name.split("").length > 15 ? "Too long" : rental.name} {rental.name.split("").length}
              </p>
            ...
            >
              ...
              <div className="flex justify-around mt-5">
                <p className="text-xs xl:text-sm mr-auto font-medium">${rental.price}/month</p>
                <p className={rental.available ? "text-xs text-sky-500" : "text-xs line-through text-red-500"}>{rental.available ? "Available" : "Sold out"}</p>
                <Button
